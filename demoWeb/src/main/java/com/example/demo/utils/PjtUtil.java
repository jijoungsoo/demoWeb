...
public class PjtUtil {
    ...
    public String encAES256AndUrl(String msg) throws Exception {
        /*
         * https://aaboo.home.blog/2019/11/29/aes-256-%EC%95%94%EB%B3%B5%ED%98%B8%ED%99%
         * 94/ url Encode를 하지 않으면 문자열이 cookie에 valid 하지 않다고 나온다.
         * 
         */
    ...
    public String encryptAES256(String msg, String key) throws Exception {
        ...
        PBEKeySpec spec = new PBEKeySpec(key.toCharArray(), saltBytes, 100, 256); // 첫번째 100 기존에 70000이었는데 너무 느려서 1000으로
                                                                                  // 변경
    ...
    public String decryptAES256(String msg, String key) throws Exception {
        ...
        SecretKeyFactory factory = SecretKeyFactory.getInstance("PBKDF2WithHmacSHA1");
        PBEKeySpec spec = new PBEKeySpec(key.toCharArray(), saltBytes, 100, 256); // 첫번째 100 기존에 70000이었는데 너무 느려서 1000으로
                                                                                  // 변경 보통 너무 적은 숫자면 해킹에 위험이 있다고 한다.
                                                                                  // 0.2초에 맞추는게 좋다고 한다.
        SecretKey secretKey = factory.generateSecret(spec);
        SecretKeySpec secret = new SecretKeySpec(secretKey.getEncoded(), "AES");
    ...
    public String makeLSession(String br, String jsonInString, Authentication authentication)
            throws JsonMappingException, JsonProcessingException {
        /*
         * 출처: https://itstory.tk/entry/Spring-Security-현재-로그인한-사용자-정보-가져오기 [덕's IT
         * Story]
         */
